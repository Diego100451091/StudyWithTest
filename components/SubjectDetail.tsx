    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
         <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <p className="text-slate-400 text-xs font-semibold uppercase">Avg Score</p>
            <p className={`text-2xl font-bold ${avgScore >= 70 ? 'text-green-600' : 'text-slate-800'}`}>{avgScore}%</p>
         </div>
         <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <p className="text-slate-400 text-xs font-semibold uppercase">Attempts</p>
         ...
         <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm cursor-pointer hover:border-amber-300 transition-colors" onClick={() => failedIdsInSubject.length > 0 && handleRunFailed(TestMode.STUDY)}>
            <p className="text-slate-400 text-xs font-semibold uppercase">Failed Questions</p>
         ...
         <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm cursor-pointer hover:border-blue-300 transition-colors" onClick={() => bookmarkedIdsInSubject.length > 0 && handleRunBookmarked(TestMode.STUDY)}>
            <p className="text-slate-400 text-xs font-semibold uppercase">Bookmarked</p>
            <div className="flex items-center justify-between">
      ...
      {tests.length > 0 && (
        <div className="sticky top-0 z-30 bg-slate-50/95 backdrop-blur py-4 border-b border-slate-200 mb-6 flex flex-wrap items-center gap-3 justify-between">
            <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-slate-500">{selectedTests.size} selected</span>
            </div>

            <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-slate-400 uppercase mr-2 hidden md:inline">Start Selected:</span>
                <button 
      ...
      <div className="space-y-3">
            ...
            >
                ...
                <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4 mt-1 text-xs text-slate-400">
                        <span className="flex items-center"><HelpCircle className="w-3 h-3 mr-1" /> {test.questions.length} questions</span>
                        <span className="flex items-center"><Clock className="w-3 h-3 mr-1" /> Added {new Date(test.createdAt).toLocaleDateString()}</span>
                    </div>
                </div>
        ...
        {tests.length === 0 && (
            <div className="text-center py-12 bg-white rounded-xl border border-dashed border-slate-200">
                ...
                <p className="text-slate-500">No tests in this subject yet.</p>
                <Link to="/ai-tools" className="text-primary hover:underline text-sm font-medium">Generate one with AI</Link>
            </div>
        )}
      </div>
      </div>
