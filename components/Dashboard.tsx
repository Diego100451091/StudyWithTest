  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{t.mySubjects}</h1>
          <p className="text-slate-500 mt-2">{t.manageSubjects}</p>
        </div>
        <button 
          onClick={openCreate}
      ...
      {data.subjects.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-slate-300">
          ...
          <h3 className="text-lg font-medium text-slate-900">{t.noSubjectsYet}</h3>
          <p className="text-slate-500 mb-6">{t.createSubjectPrompt}</p>
          <button onClick={openCreate} className="text-primary font-semibold hover:underline">{t.createFirstSubject}</button>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">{t.colorTag}</label>
